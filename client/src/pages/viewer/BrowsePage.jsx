      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ position: 'relative', marginBottom: 20 }}>
          ...
          <input
            ...
            style={{ 
              paddingLeft: lang === 'ar' ? 12 : 34, 
              paddingRight: lang === 'ar' ? 34 : 12 
            }}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                {searchResults.length} {lang === 'ar' ? 'مطابقة' : 'matched'}
              </span>
            </div>
            ...
            <div className="browse-grid" style={{ padding: 0 }}>
              {searchResults.length === 0 && !searching ? (
                <div className="empty-state">
                  ...
                  <h3>{lang === 'ar' ? 'لم يتم العثور على نتائج' : 'No SOPs found'}</h3>
                  <p>{lang === 'ar' ? 'حاول البحث عن كلمات رئيسية أخرى' : 'Try searching for different keywords'}</p>
                </div>
              ) : (
                searchResults.map(sop => (
                  <button key={sop.id} className="browse-btn" onClick={() => navigate(`/sop/${sop.id}`)}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                      ...
                      <ChevronRight size={18} style={{ opacity: 0.7, transform: lang === 'ar' ? 'rotate(180deg)' : 'none' }} />
                    </div>
                  </button>
                ))
              )}
          ...
          <>
            ...
            {path.length > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 16, flexWrap: 'wrap' }}>
                <span style={{ cursor: 'pointer', color: 'var(--brand-accent)' }} onClick={() => { setPath([]); setSops(null); }}>
                ...
                {!sops && (
                  <div className="browse-grid" style={{ padding: 0 }}>
                    {currentLevel.length === 0 ? (
                      <div className="empty-state">
                        <div className="empty-state-icon">📂</div>
                        <h3>{lang === 'ar' ? 'لا توجد فئات فرعية هنا' : 'No sub-categories here'}</h3>
                      </div>
                    ) : (
                ...
                {sops !== null && (
                  <div className="browse-grid" style={{ padding: 0 }}>
                    {sops.length === 0 ? (
                      <div className="empty-state">
                        <div className="empty-state-icon">📄</div>
                        <h3>{lang === 'ar' ? 'لا توجد إجراءات تشغيلية منشورة' : 'No published SOPs'}</h3>
                        <p>{lang === 'ar' ? 'يرجى التحقق لاحقاً' : 'Check back later'}</p>
                      </div>
                    ) : (
                      sops.map(sop => (
                        <button key={sop.id} className="browse-btn" onClick={() => navigate(`/sop/${sop.id}`)}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                            <div style={{ textAlign: lang === 'ar' ? 'right' : 'left' }}>
                              <div style={{ fontWeight: 600 }}>{sop.title}</div>
                              <div style={{ fontSize: '0.72rem', opacity: 0.75, marginTop: 3 }}>
                                {sop.docType} · v{sop.currentVersion}
                              </div>
                            </div>
                            <ChevronRight size={18} style={{ opacity: 0.7, transform: lang === 'ar' ? 'rotate(180deg)' : 'none' }} />
                          </div>
